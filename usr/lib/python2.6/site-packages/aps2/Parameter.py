...
def get_obj_by_complex_type(value, type_param):
    logger.info("get_obj_by_complex_type(value, type_param):")
    ...
    for prop in properties:
        ...
        setattr(result, prop, get_obj_by_type(value[prop], schema["structures"][name_structure]["properties"][prop]["type"]))

    logger.info("test: result.usage = " + str(result.usage))
    logger.info("test: result.limit = " + str(result.limit))

    return result


def get_obj_by_type(value, type_param):
    logger.info("get_obj_by_type(value, type_param):")
    if type_param == STRING:
        ...
        obj = get_obj_by_complex_type(value, type_param)
        return obj


class Parameter:
    def __init__(self, name_parameter, value, json):
        logger.info("Parameter.__init__():")
        aps2.printer.print_json("value = ", value)
        logger.info("json[type] = " + json["type"])
