...
class Info:
    ...
    def get_APS_API():
        if Info.__APS_API:
            return Info.__APS_API
        else:
            raise AttributeError("APS_API is not set!")

    @staticmethod
    def get_PATH_CERTIFICATE():
        if Info.__PATH_CERTIFICATE:
            return Info.__PATH_CERTIFICATE
        else:
            raise AttributeError("PATH_CERTIFICATE is not set!")
    ...
    def set_environ(environ):
        ...
        Info.__APS_API = aps2.apsapi.API(Info.__HTTP_APS_CONTROLLER_URI)
        Info.__PATH_CERTIFICATE = os.environ['APP_PATH'] + '/config/' + str(Info.__HTTP_APS_INSTANCE_ID) + '.pem'
