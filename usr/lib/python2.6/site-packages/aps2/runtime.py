...
def get_list_custom_functions():
    ...
    for oper in hpr.schema.operations:
        ...
        for parameter in re.split("/", string_parameters):
            ...
            function.list_params.append(parameter)
            if type == STRING:
...
def add_parameters_from_query_string(list_params, params_values):
...
def call_custom_func():
    ...
    list_custom_functions = get_list_custom_functions()
    # arguments = []
    for function in list_custom_functions:
        # logger.info("pat = " + str(function.pattern))
        result_re = function.pattern.match(hpr.func)
        if result_re:
            # logger.info("oooook")
            for parameter in function.list_params:
                string_parameter = result_re.group(parameter)
                logger.info(parameter + " = " + string_parameter)
                function.params_values[parameter] = string_parameter
            # logger.info("json = " + str(function.json))
            # logger.info("call_name = " + function.call_name)
            add_parameters_from_query_string(function.list_params, function.params_values)
            ...
            result = aps2.call_to_application.call_custom_function(old_resource, function)
