...
def get_list_valid_patterns():
    ...
    for oper in hpr.schema.operations:
        ...
        string_parametrs = path[len(without_params)+1:]
        # logger.info("string_parametrs = " + string_parametrs)
        # for parametr in re.split("{.*\}", string_parametrs):
        pattern_string = without_params
        for parameter in re.split("/", string_parametrs):
        ...
        list_valid_patterns.append(re.compile(pattern_string))
    return list_valid_patterns
...
def call_custom_func():
    ...
    arguments = []
    for pattern in list_valid_patterns:
        logger.info("pat = " + str(pattern))
        result_re = pattern.match(hpr.func)
        if result_re:
            logger.info("oooook")
            paramX = result_re.group('paramX')
            logger.info("paramX = " + paramX)
        else:
            logger.info("no:(")
