...
class CustomFunction:
    # For example:
    # pattern = re.compile("/calculateSomething/(?P<paramX>[a-zA-Z][0-9a-zA-Z_]*)")
...
def get_valid_custom_functions():
    ...
    for oper in hpr.schema["operations"]:
        ...
        for parameter in re.split("/", end_of_the_path):
            ...
            # type_param = hpr.schema["operations"][oper]["parameters"][parameter]["type"]
        ...
        function.main_name = str(oper)
        logger.info("pattern_string = " + pattern_string)
        logger.info("function.json = ...")
        logger.info("function.main_name = " + function.main_name)
...
def is_match_for_custom_function(valid_custom_function):
    result_re = valid_custom_function.pattern.match(hpr.func)
    if not result_re:
        return False
    logger.info("valid_custom_function.json.verb = " + valid_custom_function.json["verb"])
    if valid_custom_function.json["verb"] != hpr.REQUEST_METHOD:
        return False
...
def call_custom_func():
    ...
    valid_custom_functions = get_valid_custom_functions()
    # arguments = []
    for valid_custom_function in valid_custom_functions:
        # logger.info("pat = " + str(valid_custom_function.pattern))
        if is_match_for_custom_function(valid_custom_function):
