...
def call_custom_func():
    ...
    if custom_function.json.get("static", "false") == "false":
        ...
        # aps2.uLogging.info("call_custom_func : OLD = " + old_resource.apphost)
        # aps2.uLogging.info("call_custom_func : OLD = " + old_resource.cloudadmin)
        # aps2.uLogging.info("call_custom_func : OLD = " + old_resource.cloudpass)
...
def application(environ, start_response):
    ...
    try:
        ...
        aps2.utils.save_certificate_if_necessary(parsed_environ, function_description)
        ...
        function_caller.call(argument.value for argument in function_arguments)

        aps2.uLogging.debug("PATH_INFO = " + parsed_environ.get("PATH_INFO"))
        aps2.uLogging.debug("QUERY_STRING = " + parsed_environ.get("QUERY_STRING"))
        aps2.uLogging.debug("length(function_arguments) = " + str(len(function_arguments)))

        for arg in function_arguments:
            aps2.uLogging.debug(arg.to_string())
        result = "{}"  # TODO Fix

        if parsed_environ.get("REQUEST_METHOD") == "DELETE":
...
#########################################################33
        ...
        # else:
        #     aps2.uLogging.info("method = " + aps2.Info.Info.get_REQUEST_METHOD())
        #     aps2.uLogging.debug("It is custom function")
