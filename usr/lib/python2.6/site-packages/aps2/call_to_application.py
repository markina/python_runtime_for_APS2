...
def get_conversion_value(name, value, param_type, hpr):
    if param_type == 'string':
        ...
        logger.info("namne = " + name)
        logger.info("value = " + value)
        logger.info("param_type = " + param_type)
        ###
        # struct = hpr.APS_API.GET("/aps/2/types/?id=" + "http://aps-standard.org/types/core/resource/1.0",
        #                     None, None, hpr.PATH_CERTIFICATE)
        # logger.info("struct = ")
        # logger.info(struct)
        ###
        # logger.info("BODY:")
        # logger.info(hpr.body)
        # TODO add not simple type
        #  http://user-mgmt.demo.apsdemo.org/methods/vpses/1.0
        return value


def call_custom_function(old_resource, function, hpr):
    logger.info("FUNC --> " + function.call_name)

    method = getattr(old_resource, function.call_name)
    arguments = []
    for param in function.list_params:
        logger.info("json parameter = " + str(function.json.parameters[param.name]))

        param_type = function.json.parameters[param.name].type

        arguments.append(get_conversion_value(param.name, param.value, param_type, hpr))
