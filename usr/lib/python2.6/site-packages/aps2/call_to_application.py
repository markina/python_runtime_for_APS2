...
def call_provision(new_resource, properties, request_json):
    aps2.uLogging.info('POST --> provision()')
...
def call_retrieve(old_resource):
    aps2.uLogging.info('GET --> retrieve()')

    old_resource.retrieve()

    old_json = aps2.ServiceId.ServiceId.get_old_json()

    aps2.uLogging.debug("after retrieve: NEW = " + old_resource.cloudpass)
    aps2.uLogging.debug("after retrieve: NEW = " + old_resource.cloudadmin)
    aps2.uLogging.debug("after retrieve: NEW = " + old_resource.apphost)

    properties = aps2.ServiceId.get_properties(aps2.ServiceId.ServiceId.get_schema_json())
...
def call_configure(old_resource, new_resource, properties, request_json):
    aps2.uLogging.info('PUT --> configure()')
    properties_dictionary_new = {}
...
def call_custom_function(old_resource, function):
    aps2.uLogging.info("FUNC --> " + function.main_name)

    for param in function.parameters:
        aps2.uLogging.debug(param.to_string_without_json())
