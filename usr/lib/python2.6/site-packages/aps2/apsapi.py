...
class API:
    ...
    def call(self, verb, path, headers=None, data=None, cert=None, rheaders=None, is_json=True):
        ...
        url = self.url + path
        aps2.uLogging.debug('%s %s ...' % (verb, url))
        if headers:
            aps2.uLogging.debug('Request headers:\n\t%s' % "\t".join(["%s: %s" % (h, headers[h]) for h in headers.keys()]))
        ...
        if not headers:
            headers = dict()
            aps2.uLogging.debug("not headers")
        ...
        try:
            ...
            aps2.uLogging.debug('Status: %s' % resp.getcode())
        except AttributeError:
            # without code check resp.read() can return empty string
            aps2.uLogging.debug('Status: %s' % resp.code)

        if resp.headers:
            aps2.uLogging.debug('Response headers:\n\t%s' % "\t".join(resp.headers.headers))
