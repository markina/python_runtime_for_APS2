...
class Types:
    ...
    def get_application_services_json():
        ...
        if os.path.isfile(os.environ['APP_PATH'] + NAME_FILE_LIST_SERVICES):
            ...
            application_services = aps2.Info.Info.get_APS_API().GET(
                '/aps/2/application', None, None, Types.get_apsc_pem_certificate_path()
            )
    ...
    def get_pem_certificate_path():
        return os.environ['APP_PATH'] + '/config/' + str(aps2.Info.Info.get_HTTP_APS_INSTANCE_ID()) + '.pem'

    @staticmethod
    def get_apsc_pem_certificate_path():
        return os.environ['APP_PATH'] + '/config/' + str(aps2.Info.Info.get_HTTP_APS_INSTANCE_ID()) + '.apsc.pem'
    ...
    def save_certificate():
        ...
        try:
            ...
            with open(Types.get_pem_certificate_path(), 'w') as outfile:
                outfile.write(aps_self)
            with open(Types.get_apsc_pem_certificate_path(), 'w') as outfile:
