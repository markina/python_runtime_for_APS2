...
class CustomFunction:
    ...
    def get_parameters(self):
        ...
        for param in self.json["parameters"]:
            ...
            if kind_param == PATH:
                value_for_param = self.pattern.match(aps2.InfoParse.InfoParse.get_current_function()).group(param)
                logger.info("[" + param + "] = " + value_for_param)
            elif kind_param == QUERY:
                query_dict = dict(urlparse.parse_qsl(aps2.Info.Info.get_QUERY_STRING()))
                logger.info("[" + param + "] = " + query_dict[param])
            elif kind_param == BODY:
                logger.info("[" + param + "] = " + json.dumps(aps2.Info.Info.get_BODY_JSON(), indent=8))
            else:
